...
from scipion.constants import CLASSES_TBL, LABEL_PROPERTY, COLUMN_NAME, PROPERTIES_TBL, PROP_KEY, \
    PROP_VALUE, OBJECTS_TBL
...
def _map_master_table(conn: sqlite3.Connection, table_name: str, column_names: list[str]) -> Dict[str, str]:
    cur = conn.cursor()
...
def map_classes_table(conn: sqlite3.Connection, table_name: str = CLASSES_TBL) -> Dict[str, str]:
    return _map_master_table(conn,
                             table_name=table_name,
                             column_names=[LABEL_PROPERTY, COLUMN_NAME])


def map_properties_table(conn: sqlite3.Connection) -> Dict[str, str]:
    return _map_master_table(conn,
                             table_name=PROPERTIES_TBL,
                             column_names=[PROP_KEY, PROP_VALUE])


def _get_values_from_table(conn: sqlite3.Connection,
                          table_name: str,
    ...
    try:
        ...
        conn.close()
        raise Exception(f"Error consulting the table {table_name}. The "
                        f"introduced file may not be a SetOfTiltSeriesMovies "
                        f"Scipion sqlite file. Exception -> {e}")


def get_tsm_ts_ids(conn: sqlite3.Connection, ts_id_mapped: str) -> list[str] | None:
    """The mapped field is the c[Number] representation on the corresponding table"""
    return _get_values_from_table(conn,
                                  table_name=OBJECTS_TBL,
                                  field_name=ts_id_mapped)
